...
imsize = 1920 if torch.cuda.is_available() else 128  # use small size if no gpu
...
style_img = image_loader("bird.jpg")
content_img = image_loader("2_Train_2.jpg")
...
# style_weights = {100000, 500000, 1000000, 5000000, 10000000}
style_weights = {1000000}

content_weight = 1
for style_weight in style_weights:
  cnn = models.vgg19(pretrained=True).features.to(device).eval()
  input_img = content_img.clone()
  output = run_style_transfer(cnn, cnn_normalization_mean, cnn_normalization_std, content_img, style_img, input_img, 300, style_weight, 1)

  plt.figure()
  filename = 'style_weight_{}_content_weight_{}.jpg'.format(style_weight, content_weight)
  # print(filename)
  imshow(output, filename)
